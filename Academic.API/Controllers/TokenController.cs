...
namespace careapi.Controllers
{
    ...
    public class TokenController : ApiController
    {
        ...
        public TokenController(IWrapperRepository wrapper)
        {
            _wrapper = wrapper;
        }
        ...
        [HttpPost]
        [Route("")]
        public async Task<IHttpActionResult> Create([FromBody]TokenVM model)
        {
            try
            {
                ...
                var result = await _wrapper.Token.CreateTokenAsync(model);
        ...
        public async  Task<IHttpActionResult> Get()
        {
            try
            {
                ...
                var result = await _wrapper.Token.GetToken(tenantId);
        ...
        public async  Task<IHttpActionResult> GetById(int tokenId)
        {
            try
            {
                int tenantId = 0;
                ...
                var result = await _wrapper.Token.GetTokenById(tokenId,tenantId);
        ...
        [HttpGet]
        [Route("{tokenId}/tokenId")]
        public async Task<IHttpActionResult> Get(int tokenId)
        {
            try
            {   
                                   var tokenIdentity = new ClaimsIdentity(HttpContext.Current.User.Identity);
                ...
                var result = await _wrapper.Token.GetToken(tokenId, tenantId);
        ...
        [HttpPut]
		[Route("{tokenId}/tokenId")]
        public async Task<IHttpActionResult> Update([FromBody]TokenVM model, int tokenId)
        {
            try
            {
                var result = await _wrapper.Token.UpdateToken(tokenId, model);
        ...
        [HttpDelete]
		[Route("{tokenId}/tokenId")]
        public async Task<IHttpActionResult> Delete(int tokenId)
        {
            try
            {
                var result = await _wrapper.Token.DeleteToken(tokenId);
