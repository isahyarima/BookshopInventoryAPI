...
{
    [RoutePrefix("api/v1/title")]
    [Authorize]
    public class TitleController : ApiController
    {
        private readonly IWrapperRepository _wrapper;
        public TitleController(IWrapperRepository wrapper)
        {
            _wrapper = wrapper;
        }
        ...
        [HttpPost]
        [Route("")]
        public async Task<IHttpActionResult> Create([FromBody]TitleVM model)
        {
            try
            {
                ...
                var result = await _wrapper.Title.CreateTitleAsync(model);
        ...
        public async  Task<IHttpActionResult> Get()
        {
            try
            {
                ...
                var result = await _wrapper.Title.GetTitle(tenantId);
		...
		[HttpGet]
        [Route("{titleId}/titleId")]
        public async  Task<IHttpActionResult> GetById(int titleId)
        {
            try
            {
                int tenantId = 0;
                ...
                var result = await _wrapper.Title.GetTitleById(titleId,tenantId);
        ...
        [HttpGet]
        [Route("{titleId}/titleId")]
        public async Task<IHttpActionResult> Get(int titleId)
        {
            try
            {   
                                   var tokenIdentity = new ClaimsIdentity(HttpContext.Current.User.Identity);
                ...
                var result = await _wrapper.Title.GetTitle(titleId, tenantId);
        ...
        [HttpPut]
		[Route("{titleId}/titleId")]
        public async Task<IHttpActionResult> Update([FromBody]TitleVM model, int titleId)
        {
            try
            {
                var result = await _wrapper.Title.UpdateTitle(titleId, model);
        ...
        [HttpDelete]
		[Route("{titleId}/titleId")]
        public async Task<IHttpActionResult> Delete(int titleId)
        {
            try
            {
                var result = await _wrapper.Title.DeleteTitle(titleId);
